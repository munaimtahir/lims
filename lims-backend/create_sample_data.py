...
def create_sample_data():
    ...
    hematology = TestCategory.objects.create(
        name="Hematology", description="Blood tests and related parameters"
    )
    ...
    print(
        f"   Created {Test.objects.count()} tests with {TestParameter.objects.count()} parameters"
    )
    ...
    order1 = Order.objects.create(
        patient=patient1, ordered_by=receptionist, status="pending"
    )
    OrderItem.objects.create(order=order1, test=cbc, price=cbc.price)
    OrderItem.objects.create(order=order1, test=glucose, price=glucose.price)
    order1.calculate_total()
    order1.save()

    print(
        f"   Created {Order.objects.count()} orders with {OrderItem.objects.count()} items"
    )
