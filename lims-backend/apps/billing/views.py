...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def receipt(self, request, pk=None):
        ...
        p.drawString(50*mm, y, f"Date: {payment.payment_date.strftime('%Y-%m-%d %H:%M:%S')}")
        y -= 15*mm
        ...
        p.drawString(50*mm, y, f"Payment Method: {payment.get_payment_method_display()}")
        y -= 15*mm
        ...
        y -= 10*mm
        p.drawString(50*mm, y, f"Amount Paid: {payment.amount} {payment.order.patient.currency if hasattr(payment.order.patient, 'currency') else ''}")
        y -= 15*mm
        ...
        p.drawString(50*mm, y, f"Recorded by: {payment.recorded_by.full_name if payment.recorded_by else 'System'}")
        y -= 10*mm
