...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.WARNING("EXECUTION MODE - Changes will be committed"))
        ...
        qualitative_keywords = [
            ...
            'qualitative', 'serology', 'antibody', 'antigen'
        ]
        ...
        for test in tests_without_mappings:
            ...
            is_qualitative = any(keyword in test_name_lower for keyword in qualitative_keywords)
        ...
        self.stdout.write(f"  Qualitative tests (will use p999): {len(tests_for_p_qual)}")
        ...
        if not dry_run:
            with transaction.atomic():
                ...
                if tests_for_p_qual:
                    ...
                    if created:
                        self.stdout.write(self.style.SUCCESS("  ✓ Created parameter: p999 (qualitative)"))
                    else:
                ...
                if tests_for_p_result:
                    ...
                    if created:
                        self.stdout.write(self.style.SUCCESS("  ✓ Created parameter: p998 (general result)"))
                    else:
        ...
        # Final verification
        self.stdout.write("\n" + "="*60)
        remaining_without = Test.objects.filter(
            is_active=True
        ).exclude(
