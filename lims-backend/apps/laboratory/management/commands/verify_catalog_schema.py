...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT column_name FROM information_schema.columns WHERE table_name = 'parameters'"
            )
            columns = [row[0] for row in cursor.fetchall()]

        if "parameter_id" not in columns:
            self.stdout.write(
                self.style.ERROR("FAIL: parameters table missing parameter_id column")
            )
            ...
            self.stdout.write(
                self.style.SUCCESS("PASS: parameters.parameter_id exists")
            )
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"PASS: TestParameter queryable ({tp_count} rows)")
            )

            rr_count = ReferenceRange.objects.count()
            self.stdout.write(
                self.style.SUCCESS(f"PASS: ReferenceRange queryable ({rr_count} rows)")
            )

            p_count = Parameter.objects.count()
            self.stdout.write(
                self.style.SUCCESS(f"PASS: Parameter queryable ({p_count} rows)")
            )
