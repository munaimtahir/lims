...
class TestCatalogAuditReal:
    def test_audit_detects_existing_issues(self):
        ...
        param = Parameter.objects.create(
            parameter_id="p9999", parameter_name="Range Param", unit="mg/dl"
        )
        ...
        found_empty_test = any(
            t["test_code"] == "AUDIT001" for t in tests_missing_params
        )
        assert (
            found_empty_test
        ), "Audit failed to detect existing test without parameters"
        ...
        found_missing_range = any(
            m["test_id"] == range_test.test_id
            and m["parameter_id"] == param.parameter_id
            for m in missing_ranges
        )
        assert (
            found_missing_range
        ), "Audit failed to detect existing test parameter check missing reference range"
