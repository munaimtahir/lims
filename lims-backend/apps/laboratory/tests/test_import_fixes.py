...
class TestValidateSheetHeaders:
    ...
    def test_all_required_present(self):
        ...
        warnings = []
        missing = _validate_sheet_headers(
            "Tests", headers, ["test_id", "test_code"], warnings
        )
        assert missing == []
...
class TestRegressionGoodFile:
    ...
    def test_import_ready_file_parses(self, db):
        ...
        test_file = os.path.join(
            os.path.dirname(__file__), "../../../LIMS_TestCatalog_IMPORT_READY.xlsx"
        )
        ...
        assert (
            result["counts"]["tests"]["created"] > 0
            or result["counts"]["tests"]["unchanged"] > 0
        )
