...
class TestResultFilter(django_filters.FilterSet):
    ...
    # Date range
    entered_from = django_filters.DateTimeFilter(
        field_name="entered_at", lookup_expr="gte"
    )
    entered_to = django_filters.DateTimeFilter(
        field_name="entered_at", lookup_expr="lte"
    )
