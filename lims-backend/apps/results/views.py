...
class TestResultViewSet(viewsets.ModelViewSet):
    ...
    def worklist(self, request):
        ...
        order_items_needing_results = OrderItem.objects.filter(
            id__in=collected_samples
        ).exclude(
            results__isnull=False
        ).select_related('order', 'order__patient', 'test', 'panel')
        
        # Also include order items with pending results
        pending_results = self.queryset.filter(status='pending').values_list('order_item_id', flat=True).distinct()
    ...
    def verification_queue(self, request):
        ...
        """
        pending_results = self.queryset.filter(
            status='pending'
        ).select_related(
            'order_item', 'order_item__order', 'order_item__order__patient',
            'test_parameter', 'entered_by'
        ).order_by('entered_at')
    ...
    def bulk_entry(self, request):
        ...
        return Response(response_data, status=status.HTTP_201_CREATED if created_results else status.HTTP_400_BAD_REQUEST)
