...
class TestPatientViewSet:
    ...
    def test_create_patient(self, authenticated_client, admin_user):
        ...
        response = authenticated_client.post('/api/v1/patients/', {
            'first_name': 'Test',
            'last_name': 'Patient',
    ...
    def test_list_patients(self, authenticated_client, sample_patient):
        ...
        assert 'results' in response.data or 'data' in response.data or 'success' in response.data
    ...
    def test_update_patient(self, authenticated_client, sample_patient):
        ...
        response = authenticated_client.patch(f'/api/v1/patients/{sample_patient.id}/', {
            'phone': '03001112233'
        })
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_patient_history(self, authenticated_client, sample_patient):
        ...
        response = authenticated_client.get(f'/api/v1/patients/{sample_patient.id}/history/')
        assert response.status_code == status.HTTP_200_OK
        assert response.data['success'] is True
