...
from apps.core.numbering import generate_lab_number, generate_registration_number
...
def center_10(db):
    ...
    center, _ = CollectionCenter.objects.get_or_create(
        code="10", defaults={"name": "Test Center", "is_active": True}
    )
    return center
...
class TestLabNumbering:
    ...
    def test_lab_number_limit(self, center_00):
        ...
        counter, _ = LabDailyCounter.objects.get_or_create(
            date=dt.date(), center=center_00, defaults={"last_value": 999}
        )
    ...
    def test_order_auto_generates_lab_number(self, center_00):
        ...
        order = Order.objects.create(patient=patient, collection_center=center_00)
