...
def custom_exception_handler(exc, context):
    ...
    if status_code == 400:
        ...
        if not hasattr(context.get('request', None), 'user') or not context.get('request').user.is_staff:
            custom_response_data['detail'] = "An unexpected error occurred. Please try again later."
