#!/usr/bin/env python3
"""
LIMS v1.0 - Complete Smoke Test Script V2 (Contract Aware)
Tests all workflows end-to-end with tolerant key-name mapping.
"""
...
def log_test(step, result, message, details=None):
    ...
    if result == "FAIL":
        ...
        if details:
            # Redact common secrets if they appear in details
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if tests and len(tests) > 0:
                with open(f"{ARTIFACTS_DIR}/contract_tests_keys.json", "w") as f:
                    json.dump(list(tests[0].keys()), f)
        ...
        if response.status_code == 200:
            ...
            if sample_count == expected_count:
                log_test(
                    "REGRESSION-ISSUE1",
                    "PASS",
                    f"Samples auto-created ({sample_count}/{expected_count})",
                    f"Sample IDs: {[get_any(s, ['id', 'pk']) for s in samples]}"
                )
                return samples
            else:
                log_test(
                    "REGRESSION-ISSUE1",
                    "FAIL",
                    f"Expected {expected_count} samples, found {sample_count}",
                    response.text
                )
                return []
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if params and len(params) > 0:
                with open(f"{ARTIFACTS_DIR}/contract_params_keys.json", "w") as f:
                    json.dump(list(params[0].keys()), f)
...
def main():
    print("=" * 80)
    print("LIMS v1.0 - SMOKE TEST V2 (CONTRACT AWARE)")
    print("=" * 80)
    ...
    result_id = enter_result_bulk(labtech_token, order_item_id, param_id, "7.5")
