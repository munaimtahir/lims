...
from ..services.qc_rules import QCRulesService, QCFlag
...
async def create_result(result: ResultCreate, db: Session = Depends(get_db)):
    ...
    if result.check_previous:
        previous_result = (
            db.query(TestResult)
            .filter(
                TestResult.patient_id == result.patient_id,
                TestResult.test_name == result.test_name
            )
...
async def release_result(
    ...
    if result.has_critical_flags:
        raise HTTPException(
            status_code=400,
            detail="Cannot release result with unresolved critical flags"
        )
    
    if not result.verified:
        raise HTTPException(
            status_code=400,
            detail="Result must be verified before release"
        )
