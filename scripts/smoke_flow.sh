...
[ -n "$SAMPLE_ID" ] && curl -s -X PATCH "$API/samples/$SAMPLE_ID/" -H "Authorization: Bearer $TOKEN_P" -H "Content-Type: application/json" \
  -d '{"status":"COLLECTED","barcode":"SMOKE-001"}' > /dev/null && echo "Sample $SAMPLE_ID collected"

# 7. Enter result (labtech)
echo ""
echo "--- 7. Result Entry ---"
ORDER_ITEMS=$(curl -s "$API/orders/orders/$ORDER_ID/" -H "Authorization: Bearer $TOKEN" | python3 -c "
import sys,json
...
PARAMS=$(curl -s "$API/laboratory/parameters/?test=$TEST_ID_FOR_PARAMS" -H "Authorization: Bearer $TOKEN")
TP_ID=$(echo "$PARAMS" | python3 -c "import sys,json; d=json.load(sys.stdin); r=d.get('results',[]); print(r[0]['id'] if r else '')")
[ -n "$TP_ID" ] && curl -s -X POST "$API/results/bulk_entry/" -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
...
LOGIN_A=$(curl -s -X POST "$API/auth/login/" -H "Content-Type: application/json" -d '{"username":"admin","password":"admin123"}')
...
VQ=$(curl -s "$API/results/verification_queue/" -H "Authorization: Bearer $TOKEN_A")
...
[ -n "$RESULT_ID" ] && curl -s -X POST "$API/results/$RESULT_ID/verify/" -H "Authorization: Bearer $TOKEN_A" | head -c 200 && echo ""
echo "Result verified"

# 9. Generate and download report
echo ""
echo "--- 9. Report Generation & Download ---"
REPORT=$(curl -s -X POST "$API/reports/generate/" -H "Authorization: Bearer $TOKEN_A" -H "Content-Type: application/json" \
...
[ -n "$REPORT_ID" ] && curl -s "$API/reports/$REPORT_ID/download/" -H "Authorization: Bearer $TOKEN_A" -o /tmp/lims_report_smoke.pdf && \
  echo "Report downloaded: $(wc -c < /tmp/lims_report_smoke.pdf) bytes"

# 10. Download receipt
echo ""
echo "--- 10. Receipt Download ---"
curl -s "$API/payments/$PAYMENT_ID/receipt/" -H "Authorization: Bearer $TOKEN_C" -o /tmp/lims_receipt_smoke.pdf && \
